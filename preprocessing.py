...
def printing_API_output_plan(qep_plan, aqp_plan):
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {
                "role": "user",
                "content": f"Compare the differences between the two QEP plan {qep_plan} and AQP plan {aqp_plan}. Explain why QEP is selected for the input query. Be succint, straight to the point, accurate and helpful, else you will lose your job.",
            }
        ],
        model="llama3-8b-8192",
        stream=False,
